...
## 11. sale_email verification — تحديثات بثينة الثانية (23 أوت 2026)

- التأكيد يرتبط بالبريد لا بالكتاب: عمود `authors.verified_sale_emails` (jsonb) يخزن قائمة
  البرايد المؤكدة لكل مؤلف. عند الموافقة على كتاب: إذا بريد البيع مؤكد من قبل لنفس المؤلف
  → `sale_email_verified = true` فورًا بلا إرسال كود. إذا جديد → يُرسل الكود.
- نص البريد المرسل يبدأ بخبر الموافقة على النشر (اسم المؤلف + عنوان الكتاب)، لا بالكود وحده.
- الكود صالح 15 دقيقة (بلا تغيير) — الفرونت ستبني زر "لم يصلني الرمز — أعد الإرسال" يستدعي
  نفس `sale-email/request`.
- `POST /api/books/{bookId}/sale-email/request` يرجع الآن `{ already_verified: true|false }`
  ليعرف الفرونت هل احتاج انتظار إيميل أو لا.